...
elementos han facilitado el desarrollo de este módulo del sistema, ya
que Python los gestiona de una manera eficaz y permite su uso de uso
de una manera sencilla. 

Por otra parte, hemos elegido un sistema UNIX porque nos resulta más
cómodo a la hora de implementar el sistema. 

\subsection{Diseño de la base de datos}

Para implementar los elementos necesarios para la indexación de
documentos, como son el \textit{Posting$\_$File}, la tabla de
documentos y el diccionario de términos, se ha optado por utilizar una
base de datos relacional, siguiendo el diagrama mostrado en la Figura
\ref{BD}. Las tablas que se representan son: 
...
\begin{milista}
	...
	\item \textbf{posting$\_$file}: implementa el
          posting$\_$file. Contiene los campos \textit{term},
...
Este módulo se ha desarrollado siguiendo el enfoque multicapa, para
...
Un ejemplo de la extensibilidad del sistema es que en la capa de
presentación se han implementado dos interfaces de usuario, una en
modo gráfico y otra por línea de comandos, tal y como se detallan en
la sección \ref{manual}.  

En la capa de dominio, se almacenan las clases que implementan esta
aplicación, como son el analizador de los documentos y las clases
auxiliares que utiliza, como las cachés y un archivo de
configuración. Las clases de la capa de presentación conocen a las
clases de dominio (en concreto, al analizador), pero no al revés, por
lo que podría cambiarse la interfaz de usuario sin tener que cambiar
las clases de dominio. 

Por otra parte, en la capa de persistencia se encuentran las clases
que se encargan de almacenar la información en la base de
datos. Dichas clases implementan un patrón Agente Singleton y un
patrón fabricación pura o DAO. El primer patrón es el que se encarga
de inicializar la conexión de la base de datos si no existe ya una
instancia del agente, cerrar la conexión y ejecutar las sentencias sql
que recibe del patrón DAO. Dicho patrón se ha utilizado para
...
inicializar la conexión con la base de datos al lanzar la apliación, y
...
en la base de datos documental leyendo línea por línea. Este proceso
...
\begin{milista}
	...
	\item En cada una de las palabras de la línea, se sustituyen
          ...
          en blanco, obteniendo así una lista de palabras, que recibe
	...
	\item Si al dividir la palabra solo se obtiene una palabra de
          ...
          ''], que solo contiene una palabra de la stop$\_$list
          rodeada de espacios, lo cual no tiene sentido.  
	\item En las direcciones web o de e-mail, el tratamiento es el
          mismo: se separa la palabra por seaparadores, obteniendo una
          lista y alamcenando las palabras por separado. Esto
          soluciona el siguiente problema: si el texto no tiene una
          escritura correcta y apareciese la palabra
          ''juanro.1987@gmail.com.Hola'', con este tratamiento se
          alamcenarían las palabras ''juanro'', ''1987'', ''gmail'',
          ...
          ''juanro.1987@gmail.com.Hola'', que luego no se encontraría
          ...
          términos que se habían almacenado por separado. 
	\end{enumerate}
\end{milista}
...
opción es más eficiente en cuanto a tiempo que ir accediendo a la base
de datos por cada uno de los términos que se deben almacenar de cada
...
con dos diccionarios (en Python son equivalentes a tablas hash):
\begin{milista}
