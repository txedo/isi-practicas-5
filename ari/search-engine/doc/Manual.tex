...
en la capa de presentación del sistema, por lo que hay que introducir la sentencia (\texttt{./indexEngineCMD.py}), siendo necesario
...
\begin{figure}[h]
	...
	\caption{Ayuda para el motor de indexación usando línea de comandos}
	\label{fig:menu-ayuda}
\end{figure}

\subsection{Motor de búsqueda}
Debido a las funcionalidades que debe cubrir esta aplicación, se proporciona una interfaz gráfica de usuario (ver Figura \ref{fig:search-engine-gui}). Para ejecutarla, escriba en su terminal la sentencia \texttt{./searchEngineGUI.py} una vez que se haya situado en el directorio ``presentacion'' de la aplicación.
