...
class Kohonen:
    ...
    def inicializarNeuronas(self, datos):
        for i in datos:
            ...
            neurona = Abalone(str(valoresAtributos[0]), float(valoresAtributos[1]), float(valoresAtributos[2]), float(valoresAtributos[3]), float(valoresAtributos[4]), float(valoresAtributos[5]), float(valoresAtributos[6]), float(valoresAtributos[7]))
	...
	def variarPesoNeuronas(self, radio, abalone, factorAprendizaje):
		...
		for neurona in vecinas:
			...
			neurona.length = neurona.length + (abalone.lenght - neurona.lenght)*factorAprendizaje
